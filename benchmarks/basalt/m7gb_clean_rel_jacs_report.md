# M7gb fresh5 clean relative-pose Jacobian chain

Date: 2026-08-24 JST  
Status: **Complete source-faithful replay comparison for 584 observations.**

## Outcome

The retained bounded clean GDB capture reached the first five-state frame-4
`linearizeProblem` return and recorded 9 post-write `computeRelPose` calls.
They deduplicate to 9 non-identity TimeCam relations; the tenth expected
relation is the same-TimeCam identity branch, which does not call
`computeRelPose` and supplies zero Jacobians.  The fresh M7gb Rust replay uses
the source-faithful pre-product whitening and Eigen pair-tree kernel.

| chain used for expected weighted blocks | exact lanes | exact 2x6 blocks | total blocks |
|---|---:|---:|---:|
| direct clean relative-J chain (same-timestamp stereo uses captured clean J) | 13982/14016 | 1138 | 1168 |
| Rust same-timestamp-zero policy (cross-time uses captured clean J) | 12518/14016 | 1016 | 1168 |
...
| `same_timestamp_stereo` | 61 | 1464/1464 | 0/1464 |
| `cross_time` | 462 | 11054/11088 | 11054/11088 |
...
- Rust detail: [`target/m7gb_fresh5_detail.jsonl`](../../target/m7gb_fresh5_detail.jsonl)
- Comparator: [`m7fw_compare_clean_rel_jacs_all.py`](m7fw_compare_clean_rel_jacs_all.py)
- Comparison JSON: [`target/m7gb_clean_rel_jacs_comparison.json`](../../target/m7gb_clean_rel_jacs_comparison.json)

The Rust production source was edited and the release library/example were
rebuilt and tested.  The clean reference source was not edited, and no commit
or push was performed.  The GDB run was one bounded inferior and stopped at the
first frame-4 linearization return.
